...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Klik indikator untuk scroll ke bagian tentang
    if (scrollIndicator) {
    ...
    let currentPanel = null; // 'customer' | 'admin' | null
    let galleryTabsEl = null; // container tabs panel
    ...
    const projectGalleries = {
        ...
        project4: {
            ...
            panels: {
                customer: [
                    { src: 'img/warranty/cs1.png', alt: 'Customer - Tampilan Utama' },
                    { src: 'img/warranty/cs2.png', alt: 'Customer - Halaman 1' },
                    { src: 'img/warranty/cs3.png', alt: 'Customer - Halaman 3' },
                ],
                toko: [
                    { src: 'img/warranty/toko/t1.png', alt: 'Toko - Tampilan Utama' },
                    { src: 'img/warranty/toko/t2.png', alt: 'Toko - Halaman 1' },
                    { src: 'img/warranty/toko/t3.png', alt: 'Toko - Halaman 2' },
                    { src: 'img/warranty/toko/t4.png', alt: 'Toko - Halaman 3' },
                    { src: 'img/warranty/toko/t5.png', alt: 'Toko - Halaman 4' },
                    { src: 'img/warranty/toko/t6.png', alt: 'Toko - Halaman 5' }
                ]
            }
        },
        project5: {
        ...
        project6: {
            ...
            panels: {
                customer: [
                    { src: 'img/dapur/d1.png', alt: 'Customer - Halaman input nama' },
                    { src: 'img/dapur/d2.png', alt: 'Customer - Kategori Menu' },
                    { src: 'img/dapur/d3.png', alt: 'Customer - keranjang belanja' },
                    { src: 'img/dapur/d4.png', alt: 'Customer - checkout' },
                    { src: 'img/dapur/d5.png', alt: 'Customer - checkout' },
                    { src: 'img/dapur/d6.png', alt: 'Customer - checkout' },
                    { src: 'img/dapur/d7.png', alt: 'Customer - pembayaran' },
                    { src: 'img/dapur/d8.png', alt: 'Customer - Pembayaran berhasil' },
                
                ],
                admin: [
                    { src: 'img/dapur/admin/a1.png', alt: 'Admin - Dashboard' },
                    { src: 'img/dapur/admin/a2.png', alt: 'Admin - manajemen kategori ' },
                    { src: 'img/dapur/admin/a3.png', alt: 'Admin - manajemen Menu' },
                    { src: 'img/dapur/admin/a4.png', alt: 'Admin - Manajemen baner' },
                    { src: 'img/dapur/admin/a5.png', alt: 'Admin - Manajemen diskon' },
    ...
    function openGallery(projectId) {
        ...
        // Tabs panel jika tersedia
        setupGalleryTabsIfAny();

        // Buat thumbnail sesuai panel/normal
        buildThumbs();
    ...
    function showImage(index) {
        ...
        // Ambil sumber image sesuai panel/normal
        const images = getCurrentImages();
        if (images.length === 0) {
            galleryMain.src = '';
            galleryMain.alt = 'Tidak ada gambar untuk panel ini';
            return;
        }
    ...
    // Ambil daftar gambar aktif
    function getCurrentImages() {
        if (currentGallery && currentGallery.panels && currentPanel && currentGallery.panels[currentPanel]) {
            return currentGallery.panels[currentPanel];
        }
        return (currentGallery && currentGallery.images) ? currentGallery.images : [];
    }

    // Bangun thumbnails sesuai daftar gambar saat ini
    function buildThumbs() {
        ...
        images.forEach((image, index) => {
            ...
            probe.onload = () => {
                ...
                thumb.className = `gallery-thumb ${firstDone ? '' : 'active'}`;
    ...
    // Tabs untuk panel galeri (customer/admin)
    function setupGalleryTabsIfAny() {
        const existing = document.querySelector('.gallery-tabs');
        if (existing) existing.remove();

        if (!currentGallery || !currentGallery.panels) return;
        const keys = Object.keys(currentGallery.panels);
        if (keys.length === 0) return;

        currentPanel = keys.includes('customer') ? 'customer' : keys[0];

        const container = document.querySelector('.gallery-container');
        const titleNode = document.querySelector('.gallery-title');
        galleryTabsEl = document.createElement('div');
        galleryTabsEl.className = 'gallery-tabs';

        keys.forEach(key => {
            const btn = document.createElement('button');
            btn.className = 'gallery-tab' + (key === currentPanel ? ' active' : '');
            btn.textContent = key === 'customer' ? 'Panel Customer' : (key === 'toko' ? 'Panel Toko' : 'Panel Admin');
            btn.addEventListener('click', () => {
                if (currentPanel === key) return;
                galleryTabsEl.querySelectorAll('.gallery-tab').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                currentPanel = key;
                currentIndex = 0;
                buildThumbs();
        ...
        container.insertBefore(galleryTabsEl, titleNode.nextSibling);
    }
